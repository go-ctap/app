...
- Do not fabricate generated `ctapkit/service` operation envelopes or model DTOs to represent frontend, bridge, transport, or thrown runtime failures. If a service call returns a real generated envelope with `error`, pass that envelope through. If a call throws or fails before returning a generated envelope, keep the generated DTO `data` empty/null, store a `RuntimeErrorEnvelope` as the error, and report status/log entries through an explicit runtime-failure path rather than constructing fake `CredentialsEnvelope`, `InspectEnvelope`, or similar objects.

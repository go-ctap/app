...
export function isCardPresenceRecoveryCandidate(failure: Failure) {
  return candidateCodes.has(failure.code)
    || Boolean(failure.ctap && candidateStatuses.has(failure.ctap.statusCode));
}
...
export const operationRecovery = derived(
  ...
  ([$intent, $devices, $selectedDevice, $authenticatorStatus]): OperationRecoveryPresentation | null => {
    ...
    const canRetry = $intent.sawRemoval
      && selectedSmartCard

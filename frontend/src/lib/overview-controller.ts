...
function canAutoLoadOverview() {
  ...
  const overviewDetailsIdle =
    get(overviewBioSensor).state === "idle" && get(overviewMDS).state === "idle";

  return (
    Boolean(selector && get(authenticatorStatus).selectionId) &&
    ((screen === "lab" && inspectionState === "idle") ||
      (screen === "overview" &&
        (inspectionState === "idle" || (inspectionState === "ready" && overviewDetailsIdle))))
  );
}

...
import { m, mdsStateText, overviewStatusLabel } from "./overview-i18n.js";
import type {
  OverviewMDSState,
  OverviewRowStatus,
...
export function buildOverviewStandardPresentation(input: StandardPresentationInput): OverviewStandardPresentation {
  ...
  const capabilities: OverviewStandardCapability[] = [
    {
      ...
      value: modern.known ? modern.supported ? modern.value : m.status_unsupported() : m.not_reported(),
  ...
  const remainingCount = remaining.state === FactState.FactStateUnknown ? undefined : factInteger(remaining);
  ...
  return {
    title: standardTitle(modern.supported, factBoolean(u2f) === true, factBoolean(residentCredentials) === true),
    ...
    facts: standardFacts(input.mdsState, presence, clientPIN, userVerification, residentCredentials, credentialManagement),
    capabilities,
  };
}
...
function standardDescription(
  ...
  if (overviewFactStatus(userVerification) === "configured") parts.push(m.overview_standard_summary_uv());
  if (factBoolean(residentCredentials) === true) {
    parts.push(factBoolean(credentialManagement) === true
...
function fipsCertification(mds: LookupResult, statusReports: StatusReport[]) {
  const report = statusReports.find(({ status }) => status.toUpperCase().startsWith("FIPS140_CERTIFIED_L"));
  ...
  return [
    {
      ...
      value: factBoolean(presence) === true ? m.overview_standard_touch_value() : overviewStatusLabel(overviewFactStatus(presence)),
    ...
    },
    {
      ...
      tone: mdsState === "found" ? "positive" : mdsState === "error" ? "warning" : mdsState === "missing" ? "muted" : "neutral",
    },
  ];
}

function ownerVerification(clientPIN: ReturnType<typeof overviewFact>, userVerification: ReturnType<typeof overviewFact>) {
  ...
  if (pin === "configured") return { value: m.overview_standard_owner_pin(), tone: "positive" as const };
  if (uv === "configured") return { value: m.overview_standard_owner_uv(), tone: "positive" as const };
  if (pin === "not configured" || uv === "not configured") {
    return { value: m.status_not_configured(), tone: "warning" as const };
  }
  if (pin === "unknown" && uv === "unknown") return { value: m.not_reported(), tone: "muted" as const };
...
function passkeySummary(
  ...
  if (factBoolean(residentCredentials) === true && factBoolean(credentialManagement) === true) {
    return { value: m.overview_standard_passkeys_storage_and_management(), tone: "positive" as const };
  }
  if (factBoolean(residentCredentials) === true) {

...
<div class="json-code" data-wrap={wrap || undefined}>
  {#await highlighted then lines}
    <pre class="shiki json-code-highlighted"><code
        >{#each lines as line, lineIndex (line.id)}{#each line.tokens as token (token.offset)}<span
              style:--shiki-light={token.variants.light?.color}
              style:--shiki-dark={token.variants.dark?.color}>{token.content}</span
            >{/each}{#if lineIndex < lines.length - 1}{newline}{/if}{/each}</code
      ></pre>
  {/await}
</div>

<script lang="ts">
  ...
  let resolvedTone = $derived.by(() => {
    ...
    if (value === false || ["unsupported", "unavailable", "not_supported"].includes(raw) || readable === m.state_not_available().toLowerCase()) return "bad";
    ...
    if (["error", "invalid-session"].includes(raw)) return "bad";
    return "neutral";
  });

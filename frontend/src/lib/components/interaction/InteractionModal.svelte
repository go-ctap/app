<script lang="ts">
  ...
  import SensitivePinField from "$lib/components/interaction/SensitivePinField.svelte";
      ...
      <ModalScrollArea>
        <form class="interaction-dialog-form" onsubmit={submit}>
          ...
          {#if presentation.kind === InteractionKind.InteractionKindPIN}
            {#if presentation.pinState?.previousAttemptInvalid}
              <Alert.Root variant="destructive">
                <Alert.Title>{m.failure_pin_invalid()}</Alert.Title>
                {#if presentation.pinState.retriesRemaining != null}
                  <Alert.Description>
                    {m.interaction_pin_retries_remaining({
                      count: presentation.pinState.retriesRemaining,
                    })}
                  </Alert.Description>
                {/if}
              </Alert.Root>
          ...
          <Dialog.Footer>
            <Button variant="outline" type="button" onclick={() => answer(false, true)}
              >{m.cancel()}</Button
            >
            <Button
            ...
            >
              {presentation.kind === InteractionKind.InteractionKindPIN
                ? m.send_pin()
                : m.continue_action()}
            </Button>
          </Dialog.Footer>

<script lang="ts">
  ...
  let output = $derived.by(() => {
    ...
    if (operation.phase === "review") return credentialDeleteOutput(operation.previewEnvelope);

    if (operation.phase === "error" && operation.failedPhase === "executing") {
      return credentialDeleteOutput(operation.previewEnvelope);
    }
      ...
      {#if output}
        <dl class="passkey-delete-target">
          ...
          <div>
            <dt>{m.user_name()}</dt>
            <dd>
              {shown(output.preview.displayName)} <span>{shown(output.preview.userName)}</span>
            </dd>
          </div>

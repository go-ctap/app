...
{#snippet passkeysTableHeader()}
  <th scope="col" data-slot="expandable-data-table-disclosure-header">{m.rp_name()}</th>
  <th scope="col">{m.user_name()}</th>
  <th scope="col" class="passkeys-table-credential">{m.credential_id()}</th>
  <th scope="col" class="passkeys-table-protection" data-align="end">UV</th>
{/snippet}
    ...
    >
      {#each presentation.rows as row (row.id)}
        ...
        <ExpandableDataTable.Row
          detailsId={`passkey-row-details-${row.id}`}
          open={selected}
          disabled={false}
          columnCount={4}
        ...
        >
          {#snippet summary(triggerProps)}
            ...
            <td class="passkeys-table-credential">
              <code class="passkeys-row-credential" title={row.credentialIDHex}>
                {row.credentialIDHex}
              </code>
            </td>
            <td class="passkeys-table-protection">

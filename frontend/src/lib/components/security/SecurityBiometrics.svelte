<script lang="ts">
  ...
  function fingerprintKindLabel() {
    if (sensor?.fingerprintKind === FingerprintKind.FingerprintKindTouch) return m.security_fingerprint_touch();
    if (sensor?.fingerprintKind === FingerprintKind.FingerprintKindSwipe) return m.security_fingerprint_swipe();
    ...
    {:else}
      ...
      {#if sensor}
        <dl class="sensor-facts">
          <div><dt>{m.security_bio_modality()}</dt><dd>{modalityLabel()}</dd></div>
          <div><dt>{m.security_fingerprint_kind()}</dt><dd>{fingerprintKindLabel()}</dd></div>
          <div><dt>{m.security_capture_samples()}</dt><dd>{reportedNumber(sensor.maxCaptureSamplesRequiredForEnroll)}</dd></div>
          <div>
            <dt>{m.security_friendly_name_limit()}</dt>
            <dd>{sensor.maxTemplateFriendlyName == null ? m.not_reported() : m.reported_bytes({ count: sensor.maxTemplateFriendlyName })}</dd>
          </div>
        </dl>
      {/if}
    {/if}
  ...
  <Card.Content class="enrollment-content">
    ...
    {:else}
      {#if enrollmentsLoading && !enrollments}
        <div class="enrollment-loading" aria-busy="true" aria-label={m.security_enrollment_loading()}>
      ...
      {#if enrollments}
        ...
        {#if enrollments.enrollments.length === 0}
          <EmptyState title={m.security_no_enrollments_title()} message={m.security_no_enrollments_message()} variant="compact">
            {#snippet icon()}<Fingerprint aria-hidden="true" />{/snippet}
          </EmptyState>
        {:else}
            ...
            <Table.Root>
              ...
              <Table.Header>
                <Table.Row>
                  ...
                  <Table.Head><span class="sr-only">{m.security_enrollment_actions()}</span></Table.Head>
                </Table.Row>
              </Table.Header>
              <Table.Body>
                {#each enrollments.enrollments as enrollment (enrollment.templateIDHex)}
                  <Table.Row>
                    <Table.Cell>{enrollment.friendlyName || m.security_unnamed_enrollment()}</Table.Cell>
      ...
      <form class="rename-form" onsubmit={handleRename}>
        <Field.Field data-invalid={friendlyNameInvalid ? "true" : undefined}>
          ...
          <Field.Description>
            ...
            {#if friendlyNameLimit != null} / {m.reported_bytes({ count: friendlyNameLimit })}{/if}
          </Field.Description>
          {#if friendlyNameInvalid && friendlyNameLimit != null}
            <Field.Error>{m.security_friendly_name_too_long({ count: friendlyNameLimit })}</Field.Error>
          {/if}
        </Field.Field>

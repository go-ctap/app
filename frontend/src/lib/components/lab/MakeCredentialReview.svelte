...
<section class="lab-review" aria-labelledby="lab-make-review-title">
  <header class="lab-review-heading">
    <h3 id="lab-make-review-title">{m.lab_review_snapshot()}</h3>
  ...
  {#if preview.warnings?.length}
    <section class="lab-review-warnings" aria-labelledby="lab-make-review-warnings-title">
      <h4 id="lab-make-review-warnings-title">{m.lab_preview_warnings()}</h4>
  ...
  <dl class="lab-review-summary">
    <div><dt>{m.lab_rp_id()}</dt><dd><code>{preview.input.rp.id}</code></dd></div>
    <div><dt>{m.lab_rp_name()}</dt><dd>{preview.input.rp.name}</dd></div>
    <div><dt>{m.lab_user_name()}</dt><dd>{preview.input.user.name}</dd></div>
    <div><dt>{m.lab_display_name()}</dt><dd>{preview.input.user.displayName}</dd></div>
    <div>
      <dt>{m.lab_cose_algorithms()}</dt>
      <dd>{preview.input.pubKeyCredParams.map((parameter) => parameter.alg).join(", ")}</dd>
    </div>
    <div><dt>{m.lab_exclude_list()}</dt><dd>{preview.input.excludeList?.length ?? 0}</dd></div>
    <div class="lab-review-wide"><dt>{m.lab_options()}</dt><dd>{optionsSummary()}</dd></div>

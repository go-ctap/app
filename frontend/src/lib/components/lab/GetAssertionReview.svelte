...
<section class="lab-review" aria-labelledby="lab-get-review-title">
  ...
  <dl class="lab-review-summary">
    <div><dt>{m.lab_rp_id()}</dt><dd><code>{preview.input.rpID}</code></dd></div>
    <div><dt>{m.lab_allow_list()}</dt><dd>{preview.input.allowList?.length ?? 0}</dd></div>
    <div><dt>{m.lab_options()}</dt><dd>{optionsSummary()}</dd></div>
    <div><dt>{m.lab_extensions()}</dt><dd>{extensionNames.length ? extensionNames.join(", ") : m.lab_not_reported()}</dd></div>
  </dl>

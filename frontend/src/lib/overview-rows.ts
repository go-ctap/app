...
function vendorIdentityRows(device: DeviceReport | null) {
  ...
  return [
    row(
      ...
      valueStatus(identity?.vendor),
      textValue(identity?.vendor, value.notReported()),
    ...
    row(
      ...
      valueStatus(identity?.model),
      textValue(identity?.model, value.notReported()),
    ...
    row(
      ...
      valueStatus(identity?.serial),
      textValue(identity?.serial, value.notReported()),
    ...
    row(
      ...
      valueStatus(identity?.firmware),
      textValue(identity?.firmware, value.notReported()),
...
function vendorInterfaceRows(device: DeviceReport | null) {
  return (device?.identity?.interfaces ?? []).flatMap((interfaceReport) => {
    ...
    if (
      device?.identity?.vendor !== Vendor.VendorToken2 ||
      (interfaceReport.supported?.length ?? 0) > 0
    ) {
...
function vendorDetailRows(device: DeviceReport | null) {
  return [...yubicoDetailRows(device)];
}
...
function yubicoDetailRows(device: DeviceReport | null) {
  ...
  if (details.partNumber) {
    rows.push(
      row(
        ...
        "informational",
        details.partNumber,
        "device.identity.details.yubico.partNumber",
      ),
    );
  }
  ...
  rows.push(
    row(
      ...
      "informational",
      yubicoFormFactorLabel(details.formFactor),
      "device.identity.details.yubico.formFactor",
    ),
    yubicoBooleanRow(
      m.matrix_name_yubico_fips,
      m.matrix_desc_yubico_fips,
      details.isFIPS,
      "device.identity.details.yubico.isFIPS",
    ),
    yubicoBooleanRow(
      m.matrix_name_yubico_security_key,
      m.matrix_desc_yubico_security_key,
      details.isSecurityKey,
      "device.identity.details.yubico.isSecurityKey",
    ),
  );
  if (details.effectiveFirmware) {
    rows.push(
      row(
        "Vendor",
        m.matrix_name_yubico_effective_firmware,
        m.matrix_desc_yubico_effective_firmware,
        "informational",
        details.effectiveFirmware,
        "device.identity.details.yubico.effectiveFirmware",
      ),
    );
  }
  ...
  rows.push(
    row(
      ...
      "informational",
      String(details.autoEjectTimeout),
      "device.identity.details.yubico.autoEjectTimeout",
    ),
    row(
      "Vendor",
      ...
      "informational",
      String(details.challengeResponseTimeout),
      "device.identity.details.yubico.challengeResponseTimeout",
    ),
    yubicoBooleanRow(
      m.matrix_name_yubico_locked,
      m.matrix_desc_yubico_locked,
      details.locked,
      "device.identity.details.yubico.locked",
    ),
    yubicoCapabilitiesRow(
      m.matrix_name_yubico_fips_capable,
      m.matrix_desc_yubico_fips_capable,
      details.fipsCapable ?? [],
      "device.identity.details.yubico.fipsCapable",
    ),
    yubicoCapabilitiesRow(
      m.matrix_name_yubico_fips_approved,
      m.matrix_desc_yubico_fips_approved,
      details.fipsApproved ?? [],
      "device.identity.details.yubico.fipsApproved",
    ),
    yubicoBooleanRow(
      m.matrix_name_yubico_pin_complexity,
      m.matrix_desc_yubico_pin_complexity,
      details.pinComplexity,
      "device.identity.details.yubico.pinComplexity",
    ),
    yubicoBooleanRow(
      m.matrix_name_yubico_nfc_restricted,
      m.matrix_desc_yubico_nfc_restricted,
      details.nfcRestricted,
      "device.identity.details.yubico.nfcRestricted",
    ),
    yubicoCapabilitiesRow(
      m.matrix_name_yubico_reset_blocked,
      m.matrix_desc_yubico_reset_blocked,
      details.resetBlocked ?? [],
      "device.identity.details.yubico.resetBlocked",
    ),
  );
  if (details.fpsVersion) {
    rows.push(
      row(
        ...
        "informational",
        details.fpsVersion,
        "device.identity.details.yubico.fpsVersion",
      ),
    );
  }
  ...
  if (details.stmVersion) {
    rows.push(
      row(
        ...
        "informational",
        details.stmVersion,
        "device.identity.details.yubico.stmVersion",
      ),
    );
  }
...
function yubicoBooleanRow(
  ...
  return row(
    ...
    description,
    "informational",
    enabled ? m.status_enabled() : m.status_disabled(),
    source,
  );
}

function yubicoCapabilitiesRow(
  name: MessageText,
  description: MessageText,
  ...
  return row(
    ...
    description,
    "informational",
    inlineList(capabilities.map(capabilityLabel), value.emptyList()),
    source,
  );
}

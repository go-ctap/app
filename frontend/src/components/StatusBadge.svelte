<script lang="ts">
  ...
  let tone = $derived.by(() => {
    ...
    if (value === true || ["supported", "configured", "enabled", "available", "ready"].includes(raw) || readable === "available") return "ok";
    if (value === false || ["unsupported", "unavailable", "not_supported", "closed"].includes(raw) || readable === "not available") return "bad";
    if (["warning", "preview_only", "preview only", "opening", "running"].includes(raw) || ["warning", "preview only", "opening", "running"].includes(readable)) return "warn";
